...
{
    public override IRepositoryBase<TEntity> Add(TEntity entity)
    {
    ...
    public override IRepositoryBase<TEntity> Update(TEntity entity)
    {
        if (string.IsNullOrEmpty(entity.{{multitenant_name}}Id))
            entity.{{multitenant_name}}Id = MultiTenant.Id;
    ...
    protected override IQueryable<TEntity> Query() 
        => string.IsNullOrEmpty(MultiTenant.Id) ? base.Query() : dbContext.Set<TEntity>().Where(x => x.{{multitenant_name}}Id == MultiTenant.Id);
}